...
function get_folder_size($folder_name){
     ...
     return format_folder_size($total_size); //returns format_folder_size() function
}
//file size finish
       ...
       if (count($folder)>0)
        {
           foreach($folder as $name)
           {
                ...
                <td> <button type="button" name="view_files" data-name="'.$name.'" class="view_files btn btn-light btn-xs">View Files</button></td>
                </tr>
                ';
           }
       }
    ...
    if ($_POST["action"]=="fetch_files") {
        ...
        foreach($file_data as $file){
          if ($file==='.' OR $file==='..') {
              ...
              $output .= '
              <tr>
               <td><img src="'.$path.'" class = "img-thumbnail" height="50" width="50">
               </td>
               <td contenteditable="true" data-folder_name="'.$_POST["folder_name"].'" data-file_name="'.$file.'" class="change_file_name">'.$file.'</td>
               <td><button name="remove_file" class="remove_file btn btn-danger btn-xs" id="'.$path.'">Remove</button>
               </td>
              </tr>
              ';
          }
